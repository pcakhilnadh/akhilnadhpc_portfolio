...
const CommonBg: React.FC = () => {
  ...
  useEffect(() => {
    const updateWindowSize = () => {
      ...
      // Only set initialized if we have valid dimensions
  ...
  return (
    <div className="absolute inset-0 overflow-hidden pointer-events-none">
      ...
      {/* Only render animated elements when fully initialized */}
      {isInitialized && windowSize.width > 0 && windowSize.height > 0 && (
        <>
          {/* 3D Wireframe Ninja Robot - only render when window size is available */}
          <motion.div
              ...
              <g fill="none" stroke="currentColor" strokeWidth="1.5" className="text-muted-foreground">
                ...
                >
                  ...
                  >
                    ...
                    {/* Additional glow effect using scale instead of r */}
                    <motion.circle 
                      key="eye-glow-scale-1"
                      cx="92" 
                      cy="45" 
                      r={2}
                      fill="none" 
      ...
      {[...Array(8)].map((_, i) => (
        <motion.div
          ...
          animate={{
            rotate: [0, 360],
            x: (mousePosition.x / windowSize.width - 0.5) * (60 + i * 20),
            y: (mousePosition.y / windowSize.height - 0.5) * (40 + i * 10),
          }}
          transition={{
            rotate: {
              duration: 15 + i * 3,
      ...
      <motion.div
        ...
        animate={{
          x: (mousePosition.x / windowSize.width - 0.5) * 40,
          y: (mousePosition.y / windowSize.height - 0.5) * 20,
        }}
        transition={{
          type: "spring",
          damping: 40,
      ...
      {[...Array(15)].map((_, i) => (
        <motion.div
          ...
          animate={{
            opacity: [0.1, 0.5, 0.1],
            scale: [0.5, 1.5, 0.5],
            x: (mousePosition.x / windowSize.width - 0.5) * (20 + Math.random() * 40),
            y: (mousePosition.y / windowSize.height - 0.5) * (10 + Math.random() * 30),
          }}
          transition={{
            opacity: {
